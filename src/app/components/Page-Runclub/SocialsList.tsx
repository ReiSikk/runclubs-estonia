...
function SocialsList({ facebook, instagram, strava, website }: SocialsListProps) {
  const hasFacebook = facebook && facebook.trim() !== "";
  const hasInstagram = instagram && instagram.trim() !== "";
  const hasStrava = strava && strava.trim() !== "";
  const hasWebsite = website && website.trim() !== "";

  if (!hasFacebook && !hasInstagram && !hasStrava && !hasWebsite) {
    return null;
  }
    ...
    <>
        ...
        {hasStrava && (
            ...
            <div className={`${styles.socialIcon} ${styles.socialIcon__strava} fp`}>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 32 32" fill="#FAF3E0">
                <path
                    fill="#FAF3E0"
                    d="M14.18 2L5.9 18h4.88l3.4-6.38L17.56 18h4.84L14.18 2zm8.22 16L20 22.79L17.56 18h-3.7L20 30l6.1-12h-3.7z"
                />
                </svg>
            </div>
            <a
                ...
                rel="noopener noreferrer"
                className="txt-body"
                aria-label="Visit our Strava profile"
            >
                Strava
            </a>
            </li>
